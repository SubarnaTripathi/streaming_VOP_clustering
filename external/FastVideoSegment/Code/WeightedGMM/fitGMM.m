...
function [ model ] = fitGMM( clusters, data, dataWeights )
    ...
    if( ~exist( 'dataWeights', 'var' ) )
        ...
        [ mu, sigma, weights ] = eStep( data, dataWeights, ...
            labelsToGamma( labels, clusters ), clusters );
        ...
        while( abs( oldLikelihood - likelihood ) < 1e-5 )
            ...
            likelihood = ...
                logLikelihood( data, dataWeights, mu, sigma, weights );
        end
...
function [ mu, sigma, weights ] = eStep( data, dataWeights, gamma, ...
    clusters )
    ...
    for( cluster = 1: clusters )
        ...
        % there are no floating precision errors that make the matrix
        % asymmetrical
        ...
        if( rcond( sigma( :, :, cluster ) ) < epss )
            warning( 'fitGMM:illConditionedSigma', ...
                'Warning, degenerate cluster detected' );
