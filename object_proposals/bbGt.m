...
%     oa(i,j)= single(t)/single(u); % subarna
    
    %% new code: subarna
    w=min(dt(i,3)+dt(i,1),gt(j,3)+gt(j,1))-max(dt(i,1),gt(j,1)); if(w<=0),continue; end
    h=min(dt(i,4)+dt(i,2),gt(j,4)+gt(j,2))-max(dt(i,2),gt(i,2)); if(h<=0),continue; end
    t=w*h; if(ig(j)), u=dt(i,3)*dt(i,4); else u=dt(i,3)*dt(i,4)+gt(j,3)*gt(j,4)-t; end; %oa(i,j)=t/u;
    oa(i,j)= single(t)/single(u); % subarna
    %%
...
i=w*h; if(ig),u=dt(3)*dt(4); else u=dt(3)*dt(4)+gt(3)*gt(4)-i; end; 
oa=i/u;
%% subarna
oa = single(i)/single(u);
%%
end
