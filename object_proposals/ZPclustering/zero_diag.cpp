...
int nrhs, const mxArray* prhs[])
{
    ...
    const size_t *idims =  mxGetDimensions(INMATRIX); //subarna (int *)
    ...
    if(mxIsSparse(INMATRIX)) {   /* Sparse matrix handling */
        ...
        size_t *in_rows = mxGetIr(INMATRIX);  //subarna (int *)
        size_t *in_cols = mxGetJc(INMATRIX);  //subarna (int *)
        double *in_values = mxGetPr(INMATRIX);
        int non_zero = mxGetNzmax(INMATRIX);  //subarna
