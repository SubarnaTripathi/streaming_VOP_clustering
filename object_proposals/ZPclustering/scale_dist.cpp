 ...
 * 
 * 
 *  mex interface tofsubarna compute locally scaled distance matrix
 * 
 *  To mexify:
 *             mex  scale_dist.cpp;
 *
...
int nrhs, const mxArray* prhs[])
{
  ...
  /* get the neighbor index */
    const int K = mxGetScalar(NEIGHBORIND); //subarna
    ...
    const size_t *idims =  mxGetDimensions(DISTMATRIX);  //subarna  (int *)
    if(idims[0] != idims[1])
    ...
    /* Pointers to non zero rows, cols and values */
        size_t *distmat_rows = mxGetIr(DISTMATRIX);  //subarna (int*)
        size_t *distmat_cols = mxGetJc(DISTMATRIX);  // subarna (int*)
